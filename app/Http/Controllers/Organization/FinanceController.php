...
class FinanceController extends Controller
{
    ...
    public function analysisIndex()
    {
        ...
        return view('organization.finance.analysis-index')->with($data);
    }
    ...
    public function withdrawIndex()
    {
        ...
        return view('organization.finance.withdraw-history-index')->with($data);
    }
    ...
    {
        $withdraw = Withdraw::whereUuid($uuid)->firstOrFail();

        $invoice_name = 'receipt-' . $withdraw->transection_id. '.pdf';
        // make sure email invoice is checked.
        $customPaper = array(0, 0, 612, 792);
        $pdf = PDF::loadView('organization.finance.receipt-pdf', ['withdraw' => $withdraw])->setPaper($customPaper, 'portrait');
